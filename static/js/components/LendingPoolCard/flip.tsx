...
const FlipCard = ({
  ...
  const tarotFactory = FACTORY_DETAILS_MAP[`${pool.chainId || 250}-${pool.tarotFactoryAddress}`];
  ...
  const isDexTypeSolidly = [DEX.SOLIDLY, DEX.SPIRIT_V2, DEX.VELODROME, DEX.GLACIER, DEX.SATIN, DEX.EQUALIZERV2, DEX.EQUILIBRE, DEX.VELOCIMETER, DEX.THENA, DEX.XCAL, DEX.SOLIDLIZARD, DEX.RAMSES, DEX.STERLING, DEX.SOLIDLY_V2].includes(pool.dex);
  const minHeight = 761;
        ...
        <div
          ...
          <div className='overflow-hidden'>
            ...
            <div className='flex flex-col p-0 pt-4 text-xs space-y-1.5'>
              ...
              {(lendingPool.withdrawFee ?? 0) > 0 &&
              <div
                className='flex justify-items-center space-x-2'>
                <div>LP Withdraw Fee</div>
                <div className='border-b border-dotted opacity-20 flex-grow'></div>
                <div>{`${(lendingPool.withdrawFee || 0) / 100}`}%</div>
              </div>
              }
