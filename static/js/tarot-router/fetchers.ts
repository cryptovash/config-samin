...
import { Address, PoolDisplayDetails, PoolTokenType, SupplyVault, TEN_18, TEN_27, UserDistributionMap, ZERO } from '../types/interfaces';
...
import VoterJSON from 'abis/contracts/solidly/BaseV1Voter.json';
...
import { asyncMapParallel } from 'utils/async';
...
export async function initializeFullLendingPoolsData(this: TarotRouter): Promise<{ [key in Address]: PoolDisplayDetails}> {
  ...
  await asyncMapParallel(poolChunks, 16, async pools => {
    const poolResults = await this.doMulticall(pools.flatMap(pool => {
      ...
      const tarotPriceOracle = this.newTarotPriceOracle(FACTORY_DETAILS_MAP[`${pool.chainId || 250}-${pool.tarotFactoryAddress}`].tarotPriceOracleAddress);
      const tarotSolidlyStablePriceOracle = this.newTarotSolidlyStablePriceOracle(FACTORY_DETAILS_MAP[`${pool.chainId || 250}-${pool.tarotFactoryAddress}`].tarotPriceOracleAddress);
      ...
      if (!pool.isTarotVault) {
      ...
      } else if (vaultDetails && [VaultType.SPIRIT_V2, VaultType.SPIRIT_BOOSTED, VaultType.SOLIDEX, VaultType.OXD, VaultType.VELODROME, VaultType.GLACIER, VaultType.SATIN, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.THENA, VaultType.EQUALIZER, VaultType.EQUALIZERV2, VaultType.XCAL, VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.STERLING, VaultType.MONOLITH, VaultType.SOLIDLY_V2].includes(vaultDetails.type)) {
        const veTokenAddress = vaultDetails.veTokenAddress || ZERO_ADDRESS;
        const gaugeDepositorAddress = vaultDetails.type === VaultType.THENA ? pool.id : dexDetails.lpDepositorAddress || dexDetails.oxVoterProxyAddress || dexDetails.masterChefAddress || dexDetails.gaugeVaultProxyAddress || ZERO_ADDRESS;
        const veToken = this.newERC20(veTokenAddress);
        const voter = new Contract(dexDetails.voterAddress || ZERO_ADDRESS, VoterJSON, this.readLibrary);
        ...
        return [
          pool.stable || [VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.GLACIER, VaultType.SATIN, VaultType.STERLING, VaultType.THENA, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.MONOLITH, VaultType.SOLIDLY_V2, VaultType.EQUALIZERV2].includes(vaultDetails.type) ? [] : [tarotPriceOracle, 'getResult', [pool.id]],
          pool.stable || [VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.GLACIER, VaultType.SATIN, VaultType.STERLING, VaultType.THENA, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.MONOLITH, VaultType.SOLIDLY_V2, VaultType.EQUALIZERV2].includes(vaultDetails.type) ? [tarotSolidlyStablePriceOracle, 'getResult', [pool.id]] : [],
          [collateral, 'totalSupply', []],
          [collateral, 'getPrices', []],
          [collateralUnderlying, 'totalSupply', []],
          [pair, 'totalSupply', []],
          ...
          [],
          [],
          pool.vaultType === VaultType.MONOLITH ? [gauge, 'balanceOf', [gaugeDepositorAddress]] : [VaultType.THENA, VaultType.EQUALIZERV2].includes(pool.vaultType) ? [gauge, 'totalSupply', []] : pool.vaultType === VaultType.OXD ? [vaultToken, 'getOxTotalSupply', []] : [gauge, 'derivedSupply', []],
          [VaultType.SPIRIT_BOOSTED, VaultType.SPIRIT_V2, VaultType.THENA].includes(pool.vaultType) ? [gauge, 'rewardRate', []] : [solidlyGauge, 'rewardRate', [dexDetails.rewardsTokenAddress]],
          [VaultType.SPIRIT_BOOSTED, VaultType.SPIRIT_V2, VaultType.THENA].includes(pool.vaultType) ? [gauge, 'periodFinish', []] : [solidlyGauge, 'periodFinish', [dexDetails.rewardsTokenAddress]],
          ...
          [VaultType.SPIRIT_BOOSTED, VaultType.SPIRIT_V2, VaultType.THENA].includes(pool.vaultType) ? [gauge, 'earned', [gaugeDepositorAddress]] : [VaultType.VELODROME, VaultType.GLACIER, VaultType.SATIN, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.EQUALIZER, VaultType.EQUALIZERV2, VaultType.XCAL, VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.STERLING, VaultType.SOLIDLY_V2].includes(pool.vaultType) ? [solidlyGauge, 'earned', [dexDetails.rewardsTokenAddress, pool.id]] : [],
          [gauge, 'balanceOf', [gaugeDepositorAddress]],
          [gauge, 'derivedBalance', [gaugeDepositorAddress]],
          [VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.STERLING, VaultType.VELODROME, VaultType.GLACIER, VaultType.SATIN, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.THENA, VaultType.EQUALIZER, VaultType.EQUALIZERV2, VaultType.XCAL, VaultType.MONOLITH, VaultType.SOLIDLY_V2].includes(pool.vaultType) ? [] : [veToken, 'balanceOf', [gaugeDepositorAddress]],
          ...
          pool.vaultType === VaultType.MONOLITH ? [vaultToken, 'getRewardInfoList', []] : [],
          [VaultType.VELOCIMETER].includes(pool.vaultType) ? [voter, 'isAlive', [gaugeAddress]] : []
        ];
      } else {
        const masterChef = pool.vaultType === VaultType.ARBIDEX ? this.newArbidexMasterChef(dexDetails.masterChefAddress) : pool.vaultType === VaultType.ZIP ? this.newZipRewards(dexDetails.masterChefAddress) : [VaultType.SPOOKY_V2, VaultType.SPOOKY_V3].includes(pool.vaultType) ? this.newMasterChefV2(dexDetails.masterChefAddress) : pool.vaultType === VaultType.LIF3 ? this.newLShareRewardPool(dexDetails.masterChefAddress) : this.newMasterChef(dexDetails.masterChefAddress);
        const rewardsToken = this.newERC20(dexDetails.rewardsTokenAddress);
        const vaultToken = pool.vaultType === VaultType.ARBIDEX ? this.newArbidexVaultToken(pool.id) : pool.vaultType === VaultType.ZIP ? this.newZipVaultToken(pool.id) : pool.vaultType === VaultType.SPOOKY_V2 ? this.newSpookyV2VaultToken(pool.id) : pool.vaultType === VaultType.SPOOKY_V3 ? this.newSpookyV3VaultToken(pool.id) : this.newVaultToken(pool.id);
        ...
        return [
          ...
          [vaultToken, 'WITHDRAW_FEE', []],
          [vaultToken, 'exchangeRate', []],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : [masterChef, dexDetails.pendingRewardFunctionName, [pool.pid, pool.id]],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : [masterChef, 'totalAllocPoint', []],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : [masterChef, 'poolInfo', [pool.pid]],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : [rewardsToken, 'balanceOf', [pool.id]],
          [lpToken, 'balanceOf', [dexDetails.masterChefAddress]],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : [masterChef, dexDetails.rewardRateFunctionName, []],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [] : dexDetails.rewardEndFunctionName ? [masterChef, dexDetails.rewardEndFunctionName, []] : [],
          [VaultType.SPOOKY_V2, VaultType.SPOOKY_V3, VaultType.ZIP].includes(pool.vaultType) ? [vaultToken, 'getRewardTokenInfoWithBalances', []] : [],
          [VaultType.ARBIDEX].includes(pool.vaultType) ? [vaultToken, 'getRewardInfoList', []] : [],
          [],
          [],
          [],
          [],
    ...
    const vaultInfo = chunkify(poolResults, poolResults.length / pools.length).map(([
      ...
      withdrawFee: withdrawFee || BigNumber.from(0),
      vaultTokenExchangeRate,
      pendingRewardsInMasterChef,
      totalAllocPoint,
      poolInfo,
      ...
      rewardRate: oxRewardData ? oxRewardData.periodFinish > Math.floor(Date.now() / 1000) ? oxRewardData.rewardRate : ZERO : vaultRewardRate || (!gaugePeriodFinish || gaugePeriodFinish.gt(BigNumber.from(Math.floor(Date.now() / 1000) - (60 * 60 * 8))) ? gaugeRewardRate : ZERO) || ZERO,
      ...
      monolithRewardInfoList: monolithRewardInfoList || [],
      gaugeIsAlive: gaugeIsAlive === undefined ? true : gaugeIsAlive,
      arbidexRewardInfoList: arbidexRewardInfoList || []
    }));
    vaultInfo.forEach(v => {
      if (!v.pool.isTarotVault) {
      ...
      } else if ([VaultType.SPIRIT_V2, VaultType.SPIRIT_BOOSTED, VaultType.SOLIDEX, VaultType.OXD, VaultType.VELODROME, VaultType.GLACIER, VaultType.SATIN, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.THENA, VaultType.EQUALIZER, VaultType.EQUALIZERV2, VaultType.XCAL, VaultType.MONOLITH, VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.STERLING, VaultType.SOLIDLY_V2].includes(v.pool.vaultType)) {
        ...
        if ([VaultType.VELODROME, VaultType.GLACIER, VaultType.SATIN, VaultType.EQUILIBRE, VaultType.VELOCIMETER, VaultType.THENA, VaultType.EQUALIZER, VaultType.EQUALIZERV2, VaultType.XCAL, VaultType.SOLIDLIZARD, VaultType.RAMSES, VaultType.STERLING, VaultType.SOLIDLY_V2].includes(v.pool.vaultType)) {
          pendingRewards = v.gaugeEarned || ZERO;
        }
        ...
        if ([VaultType.SOLIDLIZARD, VaultType.SATIN, VaultType.RAMSES].includes(v.pool.vaultType)) {
        ...
        poolsWithVaultTokenInfo[v.pool.id] = {
          ...
          borrowableExchangeRate0: v.borrowableExchangeRate0,
          borrowableExchangeRate1: v.borrowableExchangeRate1,
          pendingRewards: !v.gaugeIsAlive ? BigNumber.from(0) : pendingRewards,
          pendingRewardsB: !v.gaugeIsAlive ? BigNumber.from(0) : pendingRewardsB,
          vaultTotalSupply: v.vaultTotalSupply,
          vaultRewardRate: [VaultType.SATIN].includes(v.pool.vaultType) ? v.rewardRate.div(TEN_27) : [VaultType.SOLIDLIZARD, VaultType.STERLING].includes(v.pool.vaultType) ? v.rewardRate.div(TEN_18) : v.rewardRate,
          vaultRewardRateB: v.rewardRateB,
          reinvestBounty: !v.gaugeIsAlive ? BigNumber.from(0) : reinvestBounty,
          isVaultActive: v.rewardRate.gt(0) && v.gaugeIsAlive,
          ...
          epochBegin0: v.epochBegin0,
          epochBegin1: v.epochBegin1,
          monolithRewardInfoList: v.monolithRewardInfoList
        };
      } else {
        let allocPoint = ZERO;
        if (v.poolInfo) {
        ...
        const vaultRewardRate = [VaultType.ARBIDEX].includes(v.pool.vaultType) ? ZERO : v.totalAllocPoint.isZero() ? ZERO : rewardTokensPerSecond.mul(allocPoint).div(v.totalAllocPoint);
        const pendingRewards = [VaultType.ARBIDEX].includes(v.pool.vaultType) ? ZERO : (v.pendingRewardsInMasterChef || BigNumber.from(0)).add(v.rewardsTokenBalance);
        const reinvestBounty = [VaultType.ARBIDEX].includes(v.pool.vaultType) ? ZERO : pendingRewards.mul(v.reinvestBountyFactor).div(TEN_18);
        ...
        poolsWithVaultTokenInfo[v.pool.id] = {
          ...
          reinvestFee: v.reinvestFee,
          withdrawFee: v.withdrawFee,
          isVaultActive: [VaultType.SPOOKY_V2, VaultType.SPOOKY_V3, VaultType.ZIP, VaultType.ARBIDEX].includes(v.pool.vaultType) || allocPoint.gt(0),
          ...
          arbidexRewardInfoList: v.arbidexRewardInfoList
        };
      }
    });
  });
  ...
  for (const pool of Object.values(poolsWithVaultTokenInfo)) {
    ...
    } else if (!pool.stable && pool.twapReserves) {
      const d0 = BigNumber.from(10).pow(pool.decimals0);
      const d1 = BigNumber.from(10).pow(pool.decimals1);
      const x = pool.twapReserves[0].mul(TEN_18).div(d0);
      const y = pool.twapReserves[1].mul(TEN_18).div(d1);
      pool.twapPrice = parse18(y.mul(TEN_18).div(x));
    } else {
    ...
    if (pool.isTarotVault) {
      ...
      if (
        [VaultType.SPOOKY_V2, VaultType.SPOOKY_V3, VaultType.ZIP].includes(pool.vaultType) &&
      ...
      const monolithRewardInfoArray = [];
      (pool.monolithRewardInfoList || []).forEach(rewardInfo => {
        if (rewardInfo.rewardTokenAddress !== ZERO_ADDRESS && rewardInfo.rewardRate.gt(0) && rewardInfo.periodFinish > Math.floor(Date.now() / 1000)) {
          monolithRewardInfoArray.push(rewardInfo);
        }
      });
      pool.monolithRewardInfoArray = monolithRewardInfoArray;

      const arbidexRewardInfoArray = [];
      (pool.arbidexRewardInfoList || []).forEach(rewardInfo => {
        if (rewardInfo.rewardTokenAddress !== ZERO_ADDRESS && rewardInfo.rewardRate.gt(0)) {
          arbidexRewardInfoArray.push(rewardInfo);
        }
      });
      pool.arbidexRewardInfoArray = arbidexRewardInfoArray;
      ...
      if (
        ...
        if (pool.vaultType === VaultType.MONOLITH) {
          let totalReward = BigNumber.from(0);
          pool.monolithRewardInfoArray.forEach(rewardInfo => {
            const rewardTokenPrice = tokenPriceMap[rewardInfo.rewardTokenAddress.toLowerCase()].priceUSD.value;
            const rewardRate = rewardInfo.rewardRate;
            totalReward = totalReward.add(rewardTokenPrice.mul(rewardRate));
          });
          vaultAPR = multiplier
        ...
        } else if (pool.vaultType === VaultType.ARBIDEX) {
          let totalReward = BigNumber.from(0);
          pool.arbidexRewardInfoArray.forEach(rewardInfo => {
            const rewardTokenPrice = tokenPriceMap[rewardInfo.rewardTokenAddress.toLowerCase()].priceUSD.value;
            const rewardRate = rewardInfo.rewardRate;
            totalReward = totalReward.add(rewardTokenPrice.mul(rewardRate));
          });
  ...
  const x = await Promise.all(
    lendingPools.map(lendingPool => Promise.all([
      ...
      poolsWithVaultTokenInfo[lendingPool.id].withdrawFee,
      ...
      poolsWithVaultTokenInfo[lendingPool.id].monolithRewardInfoArray,
      poolsWithVaultTokenInfo[lendingPool.id].arbidexRewardInfoArray
    ]))
  );
  ...
  for (const lendingPool of lendingPools) {
    const [
      ...
      depositFeeBps,
      boostMultiplier,
      monolithRewardInfoArray,
      arbidexRewardInfoArray
    ] = x[i];
    i++;
    ...
    if (
      [VaultType.SPOOKY_V2, VaultType.SPOOKY_V3, VaultType.ZIP].includes(lendingPool.vaultType) &&
      rewardTokenInfoWithBalances &&
    ...
    ret[lendingPool.id] = {
      ...
      dex: dex,
      pendingRewards: vaultDetails && vaultDetails.type === VaultType.ARBIDEX ? arbidexRewardInfoArray.map(rewardInfo => parseFloat(formatUnits(rewardInfo.pendingReward, rewardInfo.rewardTokenDecimals))) : vaultDetails && vaultDetails.type === VaultType.MONOLITH ? monolithRewardInfoArray.map(rewardInfo => parseFloat(formatUnits(rewardInfo.pendingReward, rewardInfo.rewardTokenDecimals))) : vaultDetails && vaultDetails.type === VaultType.SPOOKY_V3 ?
        [0, 2, 3].includes(lendingPool.pid) ? [pendingRewardB] : [pendingReward] :
        vaultDetails && vaultDetails.type === VaultType.ZIP ?
          ...
          vaultDetails && vaultDetails.type === VaultType.SPOOKY_V2 ?
            ...
            (typeof pendingReward === 'undefined') ?
              [] :
              [pendingReward],
      reinvestBounties: vaultDetails && vaultDetails.type === VaultType.ARBIDEX ? arbidexRewardInfoArray.map(rewardInfo => parseFloat(formatUnits(rewardInfo.reinvestBounty, rewardInfo.rewardTokenDecimals))) : vaultDetails && vaultDetails.type === VaultType.MONOLITH ? monolithRewardInfoArray.map(rewardInfo => parseFloat(formatUnits(rewardInfo.reinvestBounty, rewardInfo.rewardTokenDecimals))) : vaultDetails && vaultDetails.type === VaultType.SPOOKY_V3 ?
        [0, 2, 3].includes(lendingPool.pid) ? [pendingRewardB * .01] : [pendingReward * .01] :
        vaultDetails && vaultDetails.type === VaultType.ZIP ?
          ...
          vaultDetails && vaultDetails.type === VaultType.SPOOKY_V2 ?
            ...
            (typeof reinvestBounty === 'undefined') ?
              [] :
              [reinvestBounty],
      rewardsTokensDecimals: vaultDetails && vaultDetails.type === VaultType.ARBIDEX ? arbidexRewardInfoArray.map(rewardInfo => parseFloat(rewardInfo.rewardTokenDecimals.toString())) : vaultDetails && vaultDetails.type === VaultType.MONOLITH ? monolithRewardInfoArray.map(rewardInfo => parseFloat(rewardInfo.rewardTokenDecimals.toString())) : vaultDetails && vaultDetails.type === VaultType.SPOOKY_V3 ?
        [0, 2, 3].includes(lendingPool.pid) ? [18] : [18] :
        vaultDetails && vaultDetails.type === VaultType.ZIP ?
          ...
          vaultDetails && vaultDetails.type === VaultType.SPOOKY_V2 ?
            ...
            vaultDetails && vaultDetails.rewardsTokenDecimals ?
              ...
              dex && (dex as DexDetails).rewardsTokenDecimals ?
                ...
                [],
      rewardsTokensAddresses: vaultDetails && vaultDetails.type === VaultType.ARBIDEX ? arbidexRewardInfoArray.map(rewardInfo => rewardInfo.rewardTokenAddress) : vaultDetails && vaultDetails.type === VaultType.MONOLITH ? monolithRewardInfoArray.map(rewardInfo => rewardInfo.rewardTokenAddress) : vaultDetails && vaultDetails.type === VaultType.SPOOKY_V3 ?
        [0, 2, 3].includes(lendingPool.pid) ? [rewardTokenB] : [vaultDetails.rewardsTokenAddress] :
        vaultDetails && vaultDetails.type === VaultType.ZIP ?
          ...
          vaultDetails && vaultDetails.type === VaultType.SPOOKY_V2 ?
            ...
            vaultDetails && vaultDetails.rewardsTokenAddress ?
              ...
              dex && (dex as DexDetails).rewardsTokenAddress ?
                ...
                [],
      rewardsTokensSymbols: vaultDetails && vaultDetails.type === VaultType.ARBIDEX ? arbidexRewardInfoArray.map(rewardInfo => rewardInfo.rewardTokenSymbol) : vaultDetails && vaultDetails.type === VaultType.MONOLITH ? monolithRewardInfoArray.map(rewardInfo => rewardInfo.rewardTokenSymbol) : vaultDetails && vaultDetails.type === VaultType.SPOOKY_V3 ?
        [0, 2].includes(lendingPool.pid) ? ['DEUS'] : [3].includes(lendingPool.pid) ? ['SD'] : ['BOO'] :
        vaultDetails && vaultDetails.type === VaultType.ZIP ?
...
export async function getFullLendingPools(this: TarotRouter) : Promise<PoolDisplayDetails[]> {
  ...
  ]; // ['0x7a2edc2041e130d61e18eb93a32bb13c331067a0', '0x7bc34e4aae7842637b4815f90bd50ce32cfaa58a'];
