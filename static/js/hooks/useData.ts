...
export function useFullLendingPool(lendingPoolId: Address) : PoolDisplayDetails {
  const [fullLendingPool, setFullLendingPool] = useState<PoolDisplayDetails>();
  useRouterCallback(async router => setFullLendingPool(await router.getFullLendingPool(lendingPoolId)));

  return fullLendingPool;
}
