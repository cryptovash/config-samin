...
interface SortOption {
  type: SortType;
  label: string;
  fn: ((_searchTerm: string) => (a: PoolDisplayDetails, b: PoolDisplayDetails) => number);
}
...
const getSortOptions = (): SortOption[] => [
  ...
  },
  {
    ...
    label: 'Deposited LP',
    fn: _searchTerm => (a, b) => b.totalCollateralUSD - a.totalCollateralUSD
  },
  {
    type: SortType.UNLEVERAGED_APR,
    label: '1x APR',
    fn: _searchTerm => (a, b) => b.unleveragedAPR - a.unleveragedAPR
  },
  {
    type: SortType.LEVERAGED_APR,
    label: 'Leveraged APR',
    fn: _searchTerm => (a, b) => b.leveragedAPR - a.leveragedAPR
  },
  /*
  {
    type: SortType.FARMING_APR,
    label: 'Farming APR',
    fn: _searchTerm => (a, b) => Math.max(b.farmingPoolAPR[0], b.farmingPoolAPR[1]) - Math.max(a.farmingPoolAPR[0], a.farmingPoolAPR[1])
  },
  */
  {
    ...
    label: 'Utilization',
    fn: _searchTerm => (a, b) => {
      const searchTerm = _searchTerm ? _searchTerm.trim().split(' ')[0] : '';
      return searchTerm.length === 0 ?
        Math.max(b.utilization[0], b.utilization[1]) - Math.max(a.utilization[0], a.utilization[1]) :
        (b.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? b.utilization[0] : b.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? b.utilization[1] : 0) - (a.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? a.utilization[0] : a.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? a.utilization[1] : 0);
    }
  },
  {
    ...
    fn: _searchTerm => (a, b) => {
      const searchTerm = _searchTerm ? _searchTerm.trim().split(' ')[0] : '';
      return searchTerm.length === 0 ?
        Math.max(b.supplyAPR[0], b.supplyAPR[1]) - Math.max(a.supplyAPR[0], a.supplyAPR[1]) :
        (b.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? b.supplyAPR[0] : b.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? b.supplyAPR[1] : 0) - (a.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? a.supplyAPR[0] : a.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? a.supplyAPR[1] : 0);
    }
  },
  {
    ...
    fn: _searchTerm => (a, b) => {
      const searchTerm = _searchTerm ? _searchTerm.trim().split(' ')[0] : '';
      return searchTerm.length === 0 ?
        Math.max(a.borrowAPR[0], a.borrowAPR[1]) - Math.max(b.borrowAPR[0], b.borrowAPR[1]) :
        (a.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? a.borrowAPR[0] : a.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? a.borrowAPR[1] : 9999) - (b.symbol[0].toLowerCase() === searchTerm.toLowerCase() ? b.borrowAPR[0] : b.symbol[1].toLowerCase() === searchTerm.toLowerCase() ? b.borrowAPR[1] : 9999);
    }
  }
];
...
const FILTER_OPTIONS_BY_CHAIN: {[chainId: number]: FilterOption[]} = {
  [CHAIN_IDS.FANTOM]: [
    ...
    {
      type: FilterType.SPOOKY_V3,
      label: 'Spooky V3',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.SPOOKY_V3 : false
    },
    {
      type: FilterType.EQUALIZER,
      label: 'Equalizer',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.EQUALIZER : false
    },
    {
      type: FilterType.EQUALIZERV2,
      label: 'Equalizer V2',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.EQUALIZERV2 : false
    },
    {
  ...
  [CHAIN_IDS.ARBITRUM]: [
    ...
    {
      type: FilterType.RAMSES,
      label: 'Ramses',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.RAMSES : false
    },
    {
      type: FilterType.SOLIDLIZARD,
      label: 'SolidLizard',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.SOLIDLIZARD : false
    },
    {
      type: FilterType.STERLING,
      label: 'Sterling',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.STERLING : false
    },
    {
    ...
    {
      type: FilterType.SWAPFISH,
      label: 'SwapFish',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.SWAPFISH : false
    },
    {
      type: FilterType.ARBIDEX,
      label: 'ArbiDex',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.ARBIDEX : false
    }
  ],
  [CHAIN_IDS.BSC]: [
    {
    ...
    {
      type: FilterType.THENA,
      label: 'Thena',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.THENA : false
    }
  ],
  [CHAIN_IDS.ETHEREUM_MAIN_NET]: [
    ...
    {
      type: FilterType.MONOLITH,
      label: 'Monolith',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.MONOLITH : false
    },
    {
      type: FilterType.SOLIDLY_V2,
      label: 'Solidly',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.SOLIDLY_V2 : false
    }
  ],
  [CHAIN_IDS.CANTO]: [
    {
    ...
    {
      type: FilterType.VELOCIMETER,
      label: 'Velocimeter',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.VELOCIMETER : false
    }
  ],
  [CHAIN_IDS.KAVA]: [
    {
    ...
    {
      type: FilterType.EQUILIBRE,
      label: 'Équilibre',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.EQUILIBRE : false
    }
  ],
  [CHAIN_IDS.AVAX]: [
    {
    ...
    {
      type: FilterType.GLACIER,
      label: 'Glacier',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.GLACIER : false
    }
  ],
  [CHAIN_IDS.POLYGON]: [
    {
    ...
    {
      type: FilterType.SATIN,
      label: 'Satin',
      fn: a => a.vaultDetails ? a.vaultDetails.type === VaultType.SATIN : false
    }
  ]
};
...
const LendingPools = (): JSX.Element | null => {
  ...
  const [selectedSort, setSelectedSort] = React.useState<SortOption>(getSortOptions()[0]);
  ...
  React.useEffect(() => {
    setFilteredAndSortedSearchResults(R.concat(...R.partition((a: PoolDisplayDetails) => !a.poolDisabled && !a.poolDeactivated, R.sort(selectedSort.fn(searchTerm), searchResults.filter(selectedPairFilter.fn).filter(selectedFilter.fn)))));
      ...
      <div className='!ml-0 py-3 lg:py-0 lg:pr-4 relative'>
        <Menu>
          ...
          <Menu.Items className='absolute z-tarotModal flex flex-col border border-tarotBlackHaze-750 bg-tarotBlackHaze-850 py-0 shadow-lg'>
            ...
            {getSortOptions().map((sortOption, i) => (
              <Menu.Item
                key={i}>
                {() => (
                  <div
